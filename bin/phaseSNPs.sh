...
cat $name.*strand | grep [Ss]trand | sed 's/.*chr/chr/g' | awk '{print $1}' | sort | uniq > $name.flip
...
cat $name.*strand | grep [Ss]trand | sed 's/.*chr/chr/g' | awk '{print $1}' | sort | uniq> $name.exclude
...
cat $name*.strand | sed 's/.*chr/chr/g' | awk '{print $1}' | sort | uniq> $name.exclude
plink --bfile $name --exclude  $name.exclude --make-bed --out $name
