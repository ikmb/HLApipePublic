...
save = out
write.table(HLA, paste("imputation_",args[2],".HLA.data", sep=""), quote=F, sep="\t", row.names=F)
...
# CONVERT TO PLINK OUTPUT  
plink =  rbind(HLA, NUC, PROT)

write.table(plink, paste("imputation_",args[2],".data", sep=""), quote=F, sep="\t", row.names=F)
